...
Ctrl.prototype.init = function() {
    this.cfg = {               //   *** Configuration variables; user editable *** 
        ...
        coolPinInvert : true,  //Invert this GPIO pin (e.g. for Sainsmart relay board)?
        ...
        heatPinInvert : true,  //Invert this GPIO pin (e.g. for Sainsmart relay board)?
    ...
    this.PID = new PID( 0, 0, 2.5, .0035, 6, 0);       //Handle for controller PID (uni- or bi-directional depending on settings).
...
Ctrl.prototype.updateCtrlsConfig = function( ctrlsOld, ctrlsNew ) {
    ...
    for (ind = 0; ind < ctrlsOld.length; ind++ ) {
        ...
        var tprof1 = [];
        var ind2 = 0;
        //Need to check logic here to make sure this deals with additions and deletions of elements.
...
Ctrl.prototype.resetCtrl = function( ctrls, i ) {
    ...
    //Reset up GPIO pin(s) for output in BCM mode.
...
Ctrl.prototype.processActuators = function( ctrls, i ) {
    ...
    if ( ctrls[i].currState == st_RESET ) {
        utils.log( 'Skipping processing for reset on controller ' + i + ' (' + ctrls[i].cfg.name + ')...' );
        return;
    };
...
Ctrl.prototype.processPID = function( ctrls, i ) {
    ...
    utils.log( 'In processPID()...PID: ' + JSON.stringify( ctrls[i].PID ) );
